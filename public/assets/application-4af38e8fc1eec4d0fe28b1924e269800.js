...
(function() {
  var game_on, generate_sequence, getChar, loadAlphabet, num, onGameEnd, onGameReady, options, play, resources_loaded, scene_elem, seq, show_seq_elem, timebar_width, times_to_show, user, vk_id;
  ...
  play = function() {
    ...
    if ($("#game_field").length !== 0) {
      ...
      return $.get("/user.json?vk_id=" + vk_id, function(data, status, xhr) {
  ...
  getChar = function(event) {
    ...
    if (event.which !== 0 && (event.charCode !== 0 || event.keyCode !== 0)) {
      if (event.which < 32) {
        return null;
      }
  ...
  onGameReady = function() {
    var accuracy, curr_seq_index, game_loop, keys_pressed, nback_seq_index, opt, show_time, timer, _i, _len;
    ...
    curr_seq_index = 0;
    keys_pressed = [];
    show_time = 2000 + 500 * options.length;
    if ($("#timebar").length > 0) {
      timebar_width = $("#timebar").width();
      timer = window.setInterval(function() {
        var width;
        width = $("#timebar").width();
        return $("#timebar").css('width', width - 2);
      }, show_time / 100);
    }
    $(window).on('keydown', function(e) {
      ...
      switch (char) {
        case "s":
          ...
          setTimeout(function() {
            return $('.ctrl-btn[value="s"]').removeClass("active");
          }, 400);
          break;
        case "d":
          keys_pressed.push("f");
          ...
          setTimeout(function() {
            return $('.ctrl-btn[value="f"]').removeClass("active");
          }, 400);
          break;
        case "k":
          keys_pressed.push("p");
          ...
          setTimeout(function() {
            return $('.ctrl-btn[value="p"]').removeClass("active");
          }, 400);
          break;
        case "l":
          keys_pressed.push("c");
          ...
          setTimeout(function() {
            return $('.ctrl-btn[value="c"]').removeClass("active");
          }, 400);
      }
      keys_pressed.push(getChar(e));
      return console.log("Key pressed: ", getChar(e));
    });
    $(".ctrl-btn").on('click', function(e) {
      return keys_pressed.push($(this).prop("value"));
    });
    return game_loop = window.setInterval(function() {
      var k, nback_seq_elem, o, similar, v, _j, _len1;
      if (!game_on) {
      ...
      if (nback_seq_elem !== {}) {
        similar = [];
        for (k in nback_seq_elem) {
          v = nback_seq_elem[k];
          if (seq[curr_seq_index][k] === v) {
            similar.push(k);
          }
        }
        for (_j = 0, _len1 = similar.length; _j < _len1; _j++) {
          o = similar[_j];
          accuracy[o]["all"] += 1;
          if (keys_pressed.indexOf(o) !== -1) {
            accuracy[o]["right"] += 1;
          }
        }
      }
      ...
      return show_seq_elem(curr_seq_index);
    }, show_time);
  };

  onGameEnd = function(acc) {
    var k, result, v;
    if (user === null) {
    ...
    return $.post('/progress_entries.json?vk_id=' + vk_id, {
      nsteps: num,
      result: result.join(" ")
    }, function(data, response, xhr) {
      console.log("saved entry: ", data);
      return window.location.replace('/results?vk_id=' + vk_id);
    });
  };

  show_seq_elem = function(index) {
    var curr_elem, img;
    if (scene_elem !== null) {
    ...
    if ($("#timebar").length > 0) {
      ...
      return $("#timebar").text(times_to_show - index);
    }
  };

  loadAlphabet = function(count) {
    var i, load_audio, loadedAudioCounter, onSoundLoaded, rand_index, res, resources, _i, _ref;
    resources = ["А", "Г", "Е", "И", "Л", "Н", "О", "П", "Р", "С", "Т", "Ю", "Я"].map(function(val, index, arr) {
      return "/audio/" + val + ".ogg";
    });
    loadedAudioCounter = 0;
    res = [];
    load_audio = function(uri) {
      var audio;
      audio = new Audio();
  ...
  generate_sequence = function(count, options) {
    var colors, curr_elem, forms, i, opt, positions, sounds, _i, _j, _len, _ref;
    seq = [];
    if (options.indexOf("s") !== -1) {
    ...
    for (i = _i = 0, _ref = count - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
      ...
      for (_j = 0, _len = options.length; _j < _len; _j++) {
        opt = options[_j];
        if (i < num || Math.random() < 1 - 3 / count) {
          console.log("Генерируем произвольное");
          switch (opt) {
            case "s":
              curr_elem["s"] = sounds[Math.floor(Math.random() * sounds.length)];
              break;
            case "p":
              curr_elem["p"] = positions[Math.floor(Math.random() * positions.length)];
              break;
            case "f":
              curr_elem["f"] = forms[Math.floor(Math.random() * forms.length)];
              break;
            case "c":
              curr_elem["c"] = colors[Math.floor(Math.random() * colors.length)];
          }
        } else {
          console.log("Генерируем правильное");
          switch (opt) {
            case "s":
              curr_elem["s"] = seq[i - num]["s"];
              break;
            case "p":
              curr_elem["p"] = seq[i - num]["p"];
              break;
            case "f":
              curr_elem["f"] = seq[i - num]["f"];
              break;
            case "c":
              curr_elem["c"] = seq[i - num]["c"];
          }
        }
      }
      seq.push(curr_elem);
...
(function() {
  ...
  $(document).ready(function() {
    ...
    return VK.init(function() {
      ...
      $.post('/login/' + uid + '.json', function(data, status, jqXHR) {
      ...
      return window.top.location = window.top.location;
    }, '5.21');
  });

  set_close_button = function() {
    if ($(".close-button")) {
      return $(".close-button").on("click", function(event) {
        console.log($(this.parentNode));
        return $(this.parentNode).css("display", "none");
      });
    }
  };
...
(function() {
  var add_share_buttons;

  add_share_buttons = function() {
    if ($(".share-container").length > 0) {
      return $(".share-container").each(function(i, elem) {
        var share;
